...
async function downloadAudio(url) {
  const bin = findYtdlp();
  if (!bin) throw new Error('yt-dlp not found on this system. Install it or set SAFFUL_YTDLP_PATH.');
  ...
  try {
    ...
    if (result) {
      console.log('[safful-ytdlp] Direct download OK:', (require('fs').statSync(result.filePath).size / 1024).toFixed(1), 'KB');
      return result;
    }
  } catch (err) {
    ...
    // If it's a "video unavailable" type error, don't bother with proxies
    if (/unavailable|Private video|removed|age-restricted|not available/i.test(msg)) throw err;
  }

  // ── Attempt 2: Download via SOCKS5 proxy ───────────────────────────
  ...
  try {
    ...
    const result = await parseOutput(stdout);
    if (result) {
      console.log('[safful-ytdlp] Proxy download OK:', (require('fs').statSync(result.filePath).size / 1024).toFixed(1), 'KB');
      return result;
    }
  } catch (err) {
    // Invalidate proxy cache if proxy download fails
    try { fs.unlinkSync(PROXY_CACHE); } catch {}
    console.error('[safful-ytdlp] Proxy download also failed:', err.message?.slice(0, 200));
  }

  throw new Error('Download produced no file. YouTube may be blocking this server. Try setting SAFFUL_YT_COOKIES.');
}
