...
async function ensureYtdlp() {
  ...
  _downloading = (async () => {
    ...
    console.error('[safful-ytdlp] yt-dlp not found, downloading from GitHub...');
    ...
    console.error('[safful-ytdlp] Downloaded yt-dlp:', (stat.size / 1024 / 1024).toFixed(1), 'MB');
...
async function findWorkingProxy() {
  ...
  try {
    ...
    if (cached) {
      console.error('[safful-ytdlp] Testing cached proxy:', cached);
      const result = await testProxy(cached);
      if (result) { console.error('[safful-ytdlp] Cached proxy OK'); return result; }
      console.error('[safful-ytdlp] Cached proxy stale, searching...');
      try { fs.unlinkSync(PROXY_CACHE); } catch {}
    }
  } catch {}

  const proxies = await fetchProxies();
  console.error('[safful-ytdlp] Testing', proxies.length, 'proxies...');
  ...
  for (let i = 0; i < proxies.length; i += 5) {
    ...
    if (working) {
      ...
      console.error('[safful-ytdlp] Found working proxy:', proxy);
...
async function downloadAudio(url) {
  ...
  console.error('[safful-ytdlp] Trying direct download...');
  try {
    const stdout = await runYtdlp(buildArgs(url, null), 120000);
    const result = await parseOutput(stdout);
    if (result) {
      console.error('[safful-ytdlp] Direct download OK:', (fs.statSync(result.filePath).size / 1024).toFixed(1), 'KB');
      return result;
    }
  } catch (err) {
    const msg = String(err.message || '');
    console.error('[safful-ytdlp] Direct failed:', msg.slice(0, 150));
  ...
  if (getConfiguredCookies()) {
    console.error('[safful-ytdlp] Trying with cookies...');
    try {
      const stdout = await runYtdlp(buildArgs(url, null, { withCookies: true }), 120000);
      const result = await parseOutput(stdout);
      if (result) {
        console.error('[safful-ytdlp] Cookies download OK:', (fs.statSync(result.filePath).size / 1024).toFixed(1), 'KB');
        return result;
      }
    } catch (err) {
      console.error('[safful-ytdlp] Cookies failed:', String(err.message || '').slice(0, 150));
    }
  }

  // ── Attempt 3: Download via SOCKS5 proxy ───────────────────────────
  console.error('[safful-ytdlp] Falling back to proxy...');
  try {
    ...
    console.error('[safful-ytdlp] Downloading via proxy:', proxy);
    const stdout = await runYtdlp(buildArgs(url, proxy), 120000);
    const result = await parseOutput(stdout);
    if (result) {
      console.error('[safful-ytdlp] Proxy download OK:', (fs.statSync(result.filePath).size / 1024).toFixed(1), 'KB');
      return result;
    }
  } catch (err) {
    try { fs.unlinkSync(PROXY_CACHE); } catch {}
  ...
  if (getConfiguredCookies()) {
    console.error('[safful-ytdlp] Trying proxy with cookies...');
    try {
      ...
      if (proxy) {
        ...
        const result = await parseOutput(stdout);
        if (result) {
          console.error('[safful-ytdlp] Proxy+cookies OK:', (fs.statSync(result.filePath).size / 1024).toFixed(1), 'KB');
          return result;
        }
      }
    } catch (err) {
